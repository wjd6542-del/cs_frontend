<template>
  <div>
    <div class="head">
      <h3 class="h">{{ $t("솔루션사") }} <span class="c">{{ total }}</span></h3>
      <div class="tools">
        <input v-model="q" class="field !w-48" :placeholder="$t('솔루션사·코드 검색')" @keyup.enter="search" />
        <button class="btn btn-primary" @click="openNew"><i class="fa-solid fa-plus"></i> {{ $t("솔루션사 추가") }}</button>
      </div>
    </div>
    ...
    <div class="tablewrap">
      <table class="tbl">
        <thead>
          <tr><th>{{ $t("솔루션사명") }}</th><th class="c">{{ $t("상태") }}</th><th class="c w-act">{{ $t("관리") }}</th></tr>
        </thead>
        <tbody>
          <tr v-if="!rows.length"><td colspan="3"><EmptyState icon="🧩" :title="$t('솔루션사가 없어요')" :desc="$t('솔루션 응대 대상 솔루션사를 등록해요.')" :hint="$t('＋ 솔루션사 추가')" compact /></td></tr>
    ...
    <div v-if="showForm" class="drawer" @click.self="showForm = false">
      <div class="panel">
        ...
        <div class="grid">
          <BaseInput v-model="form.name" :label="$t('솔루션사명')" />
          <BaseInput v-model="form.contact_email" :label="$t('이메일')" />
...
import { solutionCompanyApi } from "@/api/cs";
...
async function reload() {
  const res = await solutionCompanyApi.list({ q: q.value || undefined, page: page.value, limit: LIMIT });
  rows.value = res.rows || [];
...
function openEdit(s) {
  editing.value = true;
  Object.assign(form, { id: s.id, name: s.name, code: s.code || "", contact_name: s.contact_name || "", contact_phone: s.contact_phone || "", contact_email: s.contact_email || "", memo: s.memo || "", is_active: !!s.is_active });
  msg.value = ""; showForm.value = true;
}
...
async function remove(s) {
  if (!await confirmDelete(`'${s.name}' 솔루션사를 삭제할까요?`)) return;
  try { await solutionCompanyApi.remove(s.id); toast.success("삭제되었습니다."); await reload(); }
